...
describe("PostgresDriver — data & indexes", () => {
  const driver = createTestDriver();
  const schema = uniqueSchema();

  beforeAll(async () => {
    await driver.createSchema(schema);
    await driver.createTable(schema, "users", [
  ...
  it("execute SELECT recupera las filas insertadas", async () => {
    const { rows } = await driver.execute(
      `SELECT name FROM "${schema}"."users" WHERE name = $1`,
      ["Alice"]
    );
  ...
  it("updateRows con where actualiza solo la fila correcta", async () => {
    ...
    const { rows } = await driver.execute(
      `SELECT email FROM "${schema}"."users" WHERE name = $1`,
      ["Carlos"]
    );
  ...
  it("updateRows sin where actualiza todas las filas", async () => {
    const before = await driver.execute(`SELECT count(*) FROM "${schema}"."users"`);
    const total = Number((before.rows[0] as { count: string }).count);

    const { rowCount } = await driver.updateRows(schema, "users", { email: "reset@example.com" });
  ...
  it("deleteRows con where elimina solo la fila correcta", async () => {
    await driver.insertRows(schema, "users", [{ name: "ToDelete", email: "del@example.com" }]);
  ...
  it("deleteRows sin where elimina todas las filas", async () => {
    ...
    const { rows } = await driver.execute(`SELECT * FROM "${schema}"."users"`);
    expect(rows.length).toBe(0);
  });
  ...
  it("createIndex crea el índice y aparece en listIndexes", async () => {
    await driver.insertRows(schema, "users", [{ name: "IndexTest", email: "idx@test.com" }]);
  ...
  it("createIndex unique rechaza duplicados en la columna", async () => {
    ...
    await driver.insertRows(schema, "users", [{ name: "UniqueA", email: "unique@test.com" }]);
    await expect(
      driver.insertRows(schema, "users", [{ name: "UniqueB", email: "unique@test.com" }])
    ).rejects.toThrow();
  });
